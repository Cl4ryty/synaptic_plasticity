...
class DoGKernel(FilterKernel):
    ...
    """
    # Initialize with the window size and two sigma values for the Gaussian functions
    ...
    def __call__(self):
        ...
        x, y = np.mgrid[-w:w+1:1, -w:w+1:1] # grid for kernel
        a = 1.0 / (2 * math.pi) # Normalization factor
        prod = x*x + y*y
        ...
        #Get the DoG filter and return it
        dog = a * (f1-f2)
...
class GaborKernel(FilterKernel):
    ...
    def __call__(self):
        ...
        # Calculate lambda and sigma based on the window size and divisor
        ...
        # Convert the Gabor filter to a tensor and return it
